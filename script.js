...
if (window.location.protocol === "file:") {
  console.warn("This project should be opened through a local HTTP server or GitHub Pages, not via file://");
}
...
map.on("load", async () => {
  try {
    ...
    const response = await fetch(assetUrl("./data/huc6-treatments.geojson"));
  ...
  for (const [key, config] of Object.entries(WATERWAYS)) {
    ...
    toggle.addEventListener("change", async () => {
      ...
      if (!loaded && toggle.checked) {
        ...
        try {
          const response = await fetch(assetUrl(`./data/${key}-waterways.geojson`));
...
async function initializeStories() {
  ...
  try {
    const response = await fetch(assetUrl("./data/story-markers.json"));
